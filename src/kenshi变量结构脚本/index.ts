...
const CharacterSchemaBase = z.object({
  ...
  属性: z
    ...
    .transform((input: Record<string, unknown>) => {
      type CoreAttrKey = 'STR' | 'DEX' | 'PER' | 'TGH' | 'WIL' | 'INT' | 'CHA';
      const defaultAttrs: Record<CoreAttrKey, number> = {
        STR: 30,
        DEX: 30,
        PER: 30,
        TGH: 30,
      ...
      const attrMap: Record<string, CoreAttrKey> = {
        力量: 'STR',
        敏捷: 'DEX',
        感知: 'PER',
        体质: 'TGH',
      ...
      const finalOutput: Record<CoreAttrKey, { 基础: number; 加成: number }> = {
        STR: { 基础: 30, 加成: 0 },
        DEX: { 基础: 30, 加成: 0 },
        PER: { 基础: 30, 加成: 0 },
        TGH: { 基础: 30, 加成: 0 },
      ...
      const isCoreAttrKey = (key: string): key is CoreAttrKey => key in defaultAttrs;

      for (const rawKey in input) {
        const mappedKey = attrMap[rawKey];
        const upperKey = rawKey.toUpperCase();
        const stdKey = mappedKey ?? (isCoreAttrKey(upperKey) ? upperKey : null);
        if (!stdKey) continue;

        const value = input[rawKey];
        if (typeof value === 'object' && value !== null && '基础' in value) {
          const detail = value as { 基础?: unknown; 加成?: unknown };
          finalOutput[stdKey] = {
            基础: Number(detail.基础) || defaultAttrs[stdKey],
            加成: Number(detail.加成) || 0,
          };
  ...
  背包: z
    .object({
      ...
      物品: z
        ...
        .transform((data: Record<string, { 数量?: number }>) => _.pickBy(data, ({ 数量 }) => (数量 ?? 0) > 0))
        .prefault({}),
    })
    .prefault({}),
});
...
const characterTransform = (data: any) => {
  ...
  if (currentMood <= 10) {
    data.临时特质['绝望'] = { 描述: '全属性-40', 消除: '心情恢复到大于10' };
  } else if (currentMood < 20) {
    data.临时特质['濒临崩溃'] = { 描述: '全属性-20', 消除: '心情恢复到20及以上' };
  } else if (currentMood < 40) {
    data.临时特质['心情低落'] = { 描述: '全属性-5', 消除: '心情恢复到40及以上' };
  ...
  allDescriptions.forEach((desc: string) => {
    if (!desc) return;
  ...
  data.心情.最大 = Math.floor(60 + finalAttrs.WIL * 1.5 + moodTraitModifier);
  ...
  const traitWeightModifier = _.sumBy(allDescriptions, (desc: string) => {
    if (!desc) return 0;
  ...
  data.背包.负重.当前 = _.round(
    _.sumBy(_.values(data.背包.物品), (item: any) => (item.重量 || 0) * (item.数量 || 0)),
    2,
  );
