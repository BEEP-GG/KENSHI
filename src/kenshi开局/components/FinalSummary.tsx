  ...
  const resolvedRaceTitle =
    data.race === 'custom_race'
      ...
      : data.subrace === 'skeleton_false_savior'
        ? '骨人-虚伪者'
        : race && ['west_hive', 'south_hive', 'dark_hive', 'skeleton'].includes(race.id) && subrace?.title
      ...
      scenario?.id === 'holy_sword'
        ...
        : {
            ...
            介绍: /利维坦狩猎之刃/.test(weaponMeta.name)
              ...
              : /染血短刀/.test(weaponMeta.name)
                ? '一柄刃口细长的短刀，刀脊残留着你自身渗出的暗红血痕，握柄装有防滑套，拔出时带着轻微金属颤鸣。'
  ...
  const updateMvuVariables = async (messageId: number | 'latest' = getCurrentMessageId()) => {
    ...
    if (data.scenario === 'monster_hunter') {
      _.set(mvuData, 'stat_data.任务系统.巨兽猎杀条目', {
        描述: '家族的宿命从不在安逸的病榻上，而在那些不可名状的怪物血肉之中。将名单上的十一头巨兽从大陆上彻底抹除。至于它们的骇貌与致命的弱点？用你的双脚去查明',
        奖励: '未知的荣耀与财富',
        主任务状态: '进行中',
        子任务: {
          狩猎惨爪兽: {
