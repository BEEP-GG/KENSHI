...
const ItemSchema = z
  .discriminatedUnion('分类', [BackpackWeaponSchema, BackpackArmorSchema, GenericItemSchema])
  .transform(data => ({ ...data, 数量: Math.max(0, data.数量) })); // 确保数量不会是负数
...
const characterTransform = data => {
    ...
    while ((match = MODIFIER_REGEX.exec(desc)) !== null) {
      ...
      if (rawAttrName === '全属性') {
        for (const attr of coreAttributes) {
          if (data.属性[attr]) data.属性[attr].加成 += value;
        }
      } else {
        const standardAttr = attrMap[rawAttrName] || rawAttrName.toUpperCase();
        if (standardAttr && data.属性[standardAttr]) data.属性[standardAttr].加成 += value;
      }
    }
  }
  ...
  allDescriptions.forEach(desc => {
    ...
    if (match && match[1]) attackModifier += parseInt(match[1].replace(/\s/g, ''), 10);
  });
...
const StrongholdSchema = z
  .object({
    ...
    仓库: z
      ...
      .prefault({}),
    设施: z.record(z.string().describe('设施标识'), StrongholdFacilitySchema).prefault({}),
...
export const Schema = z.object({
  ...
  局势: z.object({
    ...
    敌对派系: z
      .record(
        z.string().describe('派系名称'),
        z.object({ 好感度: z.coerce.number().prefault(0), 敌对原因: z.string().prefault('') }),
      )
      .prefault({}),
    友方派系: z
      .record(
        z.string().describe('派系名称'),
        z.object({ 好感度: z.coerce.number().prefault(0), 结盟原因: z.string().prefault('') }),
      )
      .prefault({}),
  }),
  任务系统: z.record(
        ...
        子任务: z
          .record(
            ...
            z
              .object({
                进度: z
                  .object({ 当前: z.coerce.number().prefault(0), 目标: z.coerce.number().prefault(1) })
                  .prefault({}),
