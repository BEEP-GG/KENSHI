import { AlertTriangle, CheckCircle, Coins, Factory, ShieldCheck, Users, Zap } from 'lucide-react';
    ...
    <div className="p-8">
        ...
        <div>
          ...
          <div className="flex items-center space-x-2 mt-2">
            <span className="text-sm text-slate-400 font-mono tracking-widest">
              {state.outposts.find(o => o.id === state.currentOutpostId)?.location || '未知区域'}
            </span>
            <span className="text-xs px-2 py-0.5 rounded-full bg-slate-800 text-amber-500 border border-amber-500/20 shadow-[0_0_10px_rgba(217,119,6,0.1)]">
              Lv.{state.outposts.find(o => o.id === state.currentOutpostId)?.level || 1}{' '}
              {{ 1: '房屋', 2: '哨站', 3: '村庄', 4: '村庄', 5: '城镇' }[
                state.outposts.find(o => o.id === state.currentOutpostId)?.level || 1
              ] || '未知'}
            </span>
          </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-10">
        <div className="glass-card p-6 rounded-xl border-l-4 border-l-emerald-500">
          ...
          <div className="text-3xl font-mono text-slate-100 mt-2">
            {activeFacilities} <span className="text-lg text-slate-500">/ {currentFacilities.length}</span>
          </div>
          <div className="text-xs text-emerald-400 mt-2 flex items-center">
            <Zap size={12} className="mr-1" /> 运行效率良好
          </div>
        </div>
        ...
        <div className="glass-card p-6 rounded-xl border-l-4 border-l-amber-500">
          ...
          <div className="mt-2 space-y-1">
            <div className="flex items-baseline justify-between gap-3">
              <span className="text-xs text-slate-500">小队资金</span>
              <span className="text-2xl font-mono text-amber-400 text-glow">{state.squadCats.toLocaleString()}</span>
            </div>
            <div className="flex items-baseline justify-between gap-3">
              <span className="text-xs text-slate-500">仓库资金</span>
              <span className="text-2xl font-mono text-amber-500 text-glow">
                {state.warehouseCats.toLocaleString()}
              </span>
            </div>
          </div>
          <div className="text-xs text-slate-400 mt-2 text-amber-500/70">分账识别开币 (Cats)</div>
        </div>
        ...
        <div className="glass-card p-6 rounded-xl border-l-4 border-l-blue-500">
          ...
          <div className="text-3xl font-mono text-slate-100 mt-2">
            {workingStaff} <span className="text-lg text-slate-500">/ {currentEmployees.length}</span>
          </div>
        ...
        <div className="glass-card p-6 rounded-xl border-l-4 border-l-red-500">
          ...
          <div className="text-xs text-red-400 mt-2 flex items-center animate-pulse">
            <AlertTriangle size={12} className="mr-1" /> 需要您的即时决策
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 glass-panel p-6 rounded-xl relative overflow-hidden">
          ...
          <h3 className="text-xl text-slate-200 mb-6 flex items-center">
            <Zap size={20} className="mr-2 text-amber-500" /> 最新生产报告 (
            {state.outposts.find(o => o.id === state.currentOutpostId)?.name})
          </h3>
          <div className="space-y-4">
            ...
            {currentFacilities.filter(f => f.status === 'active').length === 0 && (
              <div className="text-center py-10 text-slate-600 italic text-sm">当前据点无正在运行的生产设施</div>
            )}
          </div>
        </div>

        <div className="glass-panel p-6 rounded-xl">
          <h3 className="text-xl text-slate-200 mb-6 flex items-center">资产总库藏</h3>
          <div className="space-y-3">
            {Object.keys(state.resources).length > 0 ? (
              ...
              <div className="text-center py-10 text-slate-600 italic text-sm">库藏空空如也</div>
            )}
          </div>
        </div>
      </div>
