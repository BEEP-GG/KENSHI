...
import { MapPin, Shield, Users, ChevronRight, Coins, Factory, Edit3, ArrowUpCircle } from 'lucide-react';
...
export default function Outposts({ 
  ...
  const applyUpgrade = () => {
    ...
    let cost = 0;
    if (currentLevel === 1) { cost = 5; }
    else if (currentLevel === 2) { cost = 8; }
    else if (currentLevel === 3) { cost = 12; }
    else if (currentLevel === 4) { cost = 20; }
    ...
    <div className="p-8">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        {state.outposts.map((outpost) => {
          ...
          const outpostIncome = outpostFacilities.reduce((acc, fac) => {
            ...
            if (levelData && levelData.productionType === 'Cats/天') {
              const rate = typeof levelData.productionRate === 'number' 
          ...
          return (
            ...
            >
                ...
                <div>
                  <div className="flex items-center space-x-2 mb-1 text-slate-500">
                    ...
                    <span className="text-xs px-2 py-0.5 rounded-full bg-slate-800 text-amber-500 border border-amber-500/20 shadow-[0_0_10px_rgba(217,119,6,0.1)]">
                      Lv.{currentLevel} { {1: '房屋', 2: '哨站', 3: '村庄', 4: '村庄', 5: '城镇'}[currentLevel] || '未知' }
                    </span>
                  </div>
                  <div className="flex items-center">
                    ...
                    <button 
                      onClick={(e) => { e.stopPropagation(); setRenamingOutpost(outpost); setNewName(outpost.name); }}
              ...
              <div className="flex justify-between items-center pt-4 border-t border-white/5">
                <span className={`text-xs font-medium ${isActive ? 'text-amber-500' : 'text-slate-500 group-hover:text-slate-300'}`}>
        ...
        >
          ...
          <p className="text-xs text-slate-500 group-hover:text-slate-400 transition-colors">消耗: 3,000 开币 与 10 建筑材料</p>
        </div>
      </div>
        ...
        <div className="space-y-5">
          <div className="p-4 rounded-lg bg-black/40 border border-white/5 space-y-2">
            ...
            <div className="grid grid-cols-2 gap-3 pt-1">
              <div className={`p-2 rounded flex justify-between items-center text-xs ${testMode || state.cats >= 3000 ? 'bg-emerald-950/20 text-emerald-400' : 'bg-red-950/20 text-red-400'}`}>
                <span>开币资金 (cats)</span>
                <span className="font-mono">3,000 / {state.cats.toLocaleString()}</span>
              </div>
              <div className={`p-2 rounded flex justify-between items-center text-xs ${testMode || (state.resources['建筑材料'] || 0) >= 10 ? 'bg-emerald-950/20 text-emerald-400' : 'bg-red-950/20 text-red-400'}`}>
                <span>建筑材料</span>
                <span className="font-mono">10 / {state.resources['建筑材料'] || 0}</span>
              </div>
            </div>
          ...
          <div className="flex justify-end space-x-3 pt-5 border-t border-white/10">
            ...
            <button 
              disabled={!(testMode || (state.cats >= 3000 && (state.resources['建筑材料'] || 0) >= 10)) || !createName.trim() || !createLocation.trim()}
              onClick={() => {
                if (onCreateOutpost) {
              ...
              className={`px-5 py-2 rounded text-xs font-semibold tracking-wider transition-colors ${
                (testMode || (state.cats >= 3000 && (state.resources['建筑材料'] || 0) >= 10)) && createName.trim() && createLocation.trim()
            ...
            >
              {(testMode || (state.cats >= 3000 && (state.resources['建筑材料'] || 0) >= 10)) ? '确认执照与组建' : '资源不足，无法组建'}
            </button>
          </div>
        </div>
      </Modal>
      ...
      {upgradingOutpost && (() => {
        ...
        let reqEnt = 0;
        if (currentLevel === 1) { cost = 5; }
        else if (currentLevel === 2) { cost = 8; reqFacilities = 4; }
        else if (currentLevel === 3) { cost = 12; }
        else if (currentLevel === 4) { cost = 20; reqFacilities = 7; reqEnt = 3; }
            ...
            <div className="space-y-4">
              <div className="bg-black/30 p-4 border border-white/10 rounded-lg">
                ...
                <ul className="text-xs text-slate-300 list-inside list-disc space-y-1">
                  {unlockedBlueprints.length > 0 ? unlockedBlueprints.map(name => (
                    <li key={name}>{name}</li>
              ...
              <div className="space-y-2">
                ...
                <div className={`flex justify-between items-center p-2 rounded text-xs ${hasCost ? 'bg-emerald-900/20 text-emerald-400' : 'bg-red-900/20 text-red-500'}`}>
                   <span>古代科技书</span>
                   <span>x{cost} (当前: {state.resources['古代科技书'] || 0})</span>
                </div>
                
                {reqFacilities > 0 && (
                  <div className={`flex justify-between items-center p-2 rounded text-xs ${hasFac ? 'bg-emerald-900/20 text-emerald-400' : 'bg-red-900/20 text-red-500'}`}>
                     <span>设施数量</span>
                     <span>{reqFacilities} 个 (当前: {outpostFacilities.length})</span>
                  </div>
                )}
                
                {reqEnt > 0 && (
                  <div className={`flex justify-between items-center p-2 rounded text-xs ${hasEnt ? 'bg-emerald-900/20 text-emerald-400' : 'bg-red-900/20 text-red-500'}`}>
                     <span>娱乐设施数量</span>
                     <span>{reqEnt} 个 (当前: {entFacilities.length})</span>
                  </div>
                )}
              </div>
